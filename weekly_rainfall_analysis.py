...
# --- 1. CONFIGURATION ---
WEEKLY_RAINFALL_THRESHOLD_INCHES = 0.2 # Threshold for TOTAL weekly rainfall
...
airport_coordinates = {
    "KLBT": (34.6113, -79.0595), "KMEB": (34.7965, -79.3685), "KFAY": (34.9911, -78.8871),
}
...
def analyze_well_data_weekly(excel_file, well_sheet, airport_sheet):
    try:
        ...
        # *** NEW LOGIC: Resample data to a weekly frequency ***
        # 'W' stands for weekly, ending on Sunday.
        # Rainfall is summed for the week.
        # Well level is averaged for the week.
        weekly_df = df.resample('W').agg({'rainfall': 'sum', 'well_level': 'mean'}).dropna()
        
        if weekly_df.empty: return None, None
        
        max_lag_weeks = 12 # Test for a lag of up to 12 weeks (approx 3 months)
        correlations = {}
        for lag in range(max_lag_weeks + 1):
            weekly_df['lagged_rainfall'] = weekly_df['rainfall'].shift(lag)
            
            significant_rain_df = weekly_df[weekly_df['lagged_rainfall'] >= WEEKLY_RAINFALL_THRESHOLD_INCHES].dropna()

            if len(significant_rain_df) > 5: # Require at least 5 weekly data points
...
def main():
    print(f"--- RCGMP Weekly Analysis (Total Weekly Rainfall >= {WEEKLY_RAINFALL_THRESHOLD_INCHES} inches) ---")
    results_list = []
    ...
    for well_name, (airport_name, distance) in well_to_airport_map.items():
        ...
        result_data = {'Well Name': well_name, 'Closest Airport': airport_name, 'Distance (Miles)': round(distance, 2),
                       'Optimal Lag (Weeks)': 'N/A', 'Max Correlation': 'N/A'}
        if well_sheet and airport_sheet:
            lag, corr = analyze_well_data_weekly(excel_file_name, well_sheet, airport_sheet)
            if lag is not None:
                result_data['Optimal Lag (Weeks)'] = lag
    ...
    if results_list:
        ...
        output_filename = 'correlation_results_weekly.csv'
